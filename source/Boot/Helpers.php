...
/**
 * defines the limit of attempts that the user can make in x seconds
 * @param string $key
 * @param integer $attemptLimit
 * @param integer $seconds
 * @return boolean
 */
function request_limit(string $key, int $attemptLimit = 5, int $seconds = 60): bool
{
    $session = new Session();

    if($session->has($key) && $session->$key->time >= time() && $session->$key->requests < $attemptLimit){
        $session->set($key, [
            'time' =>  time() + $seconds,
...
function request_repeat(string $field, string $value): bool
{
    ...
    if($session->has($field) && in_array($value, (array) $session->$field)){
        return true;
    }

    if($session->has($field)){
        $values = (array) $session->$field;
    }
